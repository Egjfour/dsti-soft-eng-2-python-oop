...
class ItemWithHoldingCapacity(Item):
    __slots__ = ['_items', '_max_weight', '__total_weight']

    def __init__(self, max_weight, name = None):
        # get class name to dynamically name our child class as an item
    ...
    def add_item(self, item: Item):
        if self.weight() + item.weight() > self.max_weight:
            print(f"Item {item.name()} with weight {item.weight()} kg is too heavy to add to {self.name()}. " +
